...
# Show combined Landmarks to Determine Which Code Chunks to Run
```{r}
...
#Algorithmic changes to human landmarks (Only if diff is still non zero after ratio correction ) 
```{r}
# make changes
temp = modifylm(ref.h.c, ref.h.a2, num.h.c, nrow(GPA.c2$consensus))
...
#Algorithmic changes to macaque landmarks (Only if diff is still non zero after ratio correction ) 
```{r}
#make changes
temp = modifylm(ref.m.c, ref.m.a2, num.m.c, nrow(GPA.c2$consensus))
