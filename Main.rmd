...
GPA was performed by prioritizing configuratiosn that minimize bending energy rather than procrustes distance in order to create continuous sulcal curves. The differences between the two methods are well explained [here](https://pmc.ncbi.nlm.nih.gov/articles/PMC2100233/)
...
Centroid size is a surrogate measure for overall siz calculated by the sum of the square of distance from the center of the specimen (centroid) to each landmark. The larger the brain, the more lateral sulci will be, and thus the distance from each landmark to the centroid increases.
...
# ANOVA
if(species == 'combined') {
  fit = multivariateAnalysis(GPA, species=TRUE)
} else { fit = multivariateAnalysis(GPA)}
...
```{r}
plotAllometry(fit, size=gdf$Csize, logsz=TRUE, method='PredLine', col=brainColors, pch=c(0,1)[gdf$species])
legend('topright', pch=c(0,1), legend=unique(GPA$species))
legend('bottomleft', fill=colorcor$colors, cex=0.6, legend=colorcor$age)


plotAllometry(fit, size=gdf$Csize, logsz=TRUE, method='RegScore',col=brainColors, pch=c(0,1)[gdf$species])
legend('topright', pch=c(0,1), legend=unique(GPA$species))
legend('bottomleft', fill=colorcor$colors, cex=0.6, legend=colorcor$age)
```
