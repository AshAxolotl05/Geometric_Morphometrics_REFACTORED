...
# GPA using combined reference landmarks

resampled.m = resampleCurves(mSulc, surface, 'combined', sulcalLengths=ref)
num.m = sumlm(ref)

resampled.h = resampleCurves(hSulc, surface, 'combined', sulcalLengths=ref)
num.h = sumlm(ref)
...
# GPA
GPA.m = procrustes(num.m, data.m, ref)
GPA.h = procrustes(num.h, data.h, ref)

```

```{r}
# Rotate Onto Eachother So they face same way
GPA.c = rotonto(GPA.m$consensus, GPA.h$consensus) 

# Combine Landmarks
landmarks = abind(GPA.c$X, GPA.c$Y, along = 3)

GPA.c2 = gpagen(landmarks)

#Verify everything looks right 
plot3d(GPA.c2$consensus)
points3d(GPA.c$X, col = 'blue') #Macaque
points3d(GPA.c$Y, col = 'orange') #Human

```

#Get All Macaque and Human Sulci
```{r}
mSulc.a = df[df$species=='macaque' & df$gm_or_wm==surface,]
hSulc.a = df[df$species=='human' & df$gm_or_wm==surface,]

# first GPA
resampled.m.a = resampleCurves(mSulc.a, surface, 'macaque')
num.m.a = 2 * length(getSulci(surface, 'macaque')) * 100

if(surface == 'pi') {
  num.m.a = num.m.a + 300
}

data.m.a = transformToArray(resampled.m.a,num.m.a)
GPA.m.a = procrustes(num.m.a, data.m.a)

resampled.h.a = resampleCurves(hSulc.a, surface, 'human')
num.h.a = 2 * length(getSulci(surface, 'human')) * 100

if(surface == 'pi') {
  num.h.a = num.h.a + 300
}

data.h.a = transformToArray(resampled.h.a, num.h.a)
GPA.h.a = procrustes(num.h.a, data.h.a)

#Second GPA
ref.m.a = findSulcalLengths(surface, 'macaque', GPA.m.a$consensus, ce=20)
resampled.m.a = resampleCurves(mSulc.a, surface, 'macaque', sulcalLengths=ref.m.a)
num.m.a = sumlm(ref.m.a)
data.m.a = transformToArray(resampled.m.a,num.m.a)

ref.h.a = findSulcalLengths(surface, 'human', GPA.h.a$consensus, ce=20)
resampled.h.a = resampleCurves(hSulc.a, surface, 'human', sulcalLengths=ref.h.a)
num.h.a = sumlm(ref.h.a)
data.h.a = transformToArray(resampled.h.a, num.h.a)
```
```{r}
#get only combined lm
unique = getSulci(surface, 'macaque', oneSpec=T)
num.m.c = num.m.a
for(hemi in c('lh', 'rh')){
  for(sulcus in unique){
    num.m.c = num.m.c - ref.m.a[[hemi]][[sulcus]]
  }
}

num.h.c = num.h.a
unique = getSulci(surface, 'human', oneSpec=T)
for(hemi in c('lh', 'rh')){
  for(sulcus in unique){
    num.h.c = num.h.c - ref.h.a[[hemi]][[sulcus]]
  }
}

# check against combined
print(num.h.c - nrow(GPA.c2$consensus))
print(num.m.c - nrow(GPA.c2$consensus))

```

# Fix Human Ratio (Only if diff is non zero)
```{r}
new.h.rat = 781.2492
new.h.rat = (new.h.rat * nrow(GPA.c2$consensus))/num.h.c

ref.h.a2 = findSulcalLengths(surface, 'human', GPA.h.a$consensus, rat = new.h.rat)
resampled.h.a2 = resampleCurves(hSulc.a, surface, 'human', sulcalLengths=ref.h.a2)
num.h.a2 = sumlm(ref.h.a2)

# get combined reference matrix
ref.h.c = hashmap()
unique = getSulci(surface, 'human', oneSpec = T)
#handle left and right hemispheres
  for (hemi in keys(ref.h.a2)) {
    sulcMap = hashmap()
    for (sulcus in keys(ref.h.a2[[hemi]])) {
      if (!sulcus %in% unique) {
        sulcMap[[sulcus]] = ref.h.a2[[hemi]][[sulcus]]
      }
    }
    ref.h.c[[hemi]] = sulcMap
  }

num.h.c = sumlm(ref.h.c)

# check against combined again
print((num.h.c - nrow(GPA.c2$consensus)))

```

#Fix Macaque Ratio (Only if Diff is non zero)

```{r}
new.m.rat = 890.3887
new.m.rat = (new.m.rat * nrow(GPA.c2$consensus))/num.m.c

ref.m.a2 = findSulcalLengths(surface, 'macaque', GPA.m.a$consensus, rat = new.m.rat)

for(hemi in c('lh', 'rh')){
  for(key in keys(ref.m.a2[[hemi]])){
    if(ref.m.a2[[hemi]][[key]] == 2) {
      ref.m.a2[[hemi]][[key]] = ref.m.a2[[hemi]][[key]] + 1
    }
  }
}
resampled.m.a2 = resampleCurves(mSulc.a, surface, 'macaque', sulcalLengths=ref.m.a2)
num.m.a2 = sumlm(ref.m.a2)

# get combined reference matrix
ref.m.c = hashmap()
unique = getSulci(surface, 'macaque', oneSpec = T)
#handle left and right hemispheres
  for (hemi in keys(ref.m.a2)) {
    sulcMap = hashmap()
    for (sulcus in keys(ref.m.a2[[hemi]])) {
      if (!sulcus %in% unique) {
        sulcMap[[sulcus]] = ref.m.a2[[hemi]][[sulcus]]
      }
    }
    ref.m.c[[hemi]] = sulcMap
  }

num.m.c = sumlm(ref.m.c)
...
#Algorithmic changes to human landmarks (Only if diff is still non zero after ratio correction ) HUMAN
```{r}
#make any changes to ref.h.a
diff = abs(nrow(GPA.c2$consensus) - num.h.c) # human should be smaller

# find diff # of longest sulci
max = data.frame(points = c(), hemi = c(), sulcus = c())
  
  for(hemi in c('lh', 'rh')) {
    for(sulcus in r2r::keys(ref.h.c[[hemi]])){
      points = ref.h.c[[hemi]][[sulcus]]
...
#increment them
  for(i in seq(1, abs(diff))) {
    val = max[i,]
    ref.h.a2[[val[[2]]]][[val[[3]]]] = ref.h.a2[[val[[2]]]][[val[[3]]]] + 1
    ref.h.c[[val[[2]]]][[val[[3]]]] = ref.h.c[[val[[2]]]][[val[[3]]]] + 1
    }
  
# check landmarks
  num.h.c = sumlm(ref.h.c)
  print(num.h.c - nrow(GPA.c2$consensus)) #should be 0

```

#Algorithmic changes to macaque landmarks (Only if diff is still non zero after ratio correction ) MACAQUE
```{r}
#make any changes to ref.m.a
diff = abs(nrow(GPA.c2$consensus) - num.m.c) # human should be smaller

# find diff # of longest sulci
max = data.frame(points = c(), hemi = c(), sulcus = c())
  
  for(hemi in c('lh', 'rh')) {
    for(sulcus in r2r::keys(ref.m.c[[hemi]])){
      points = ref.m.c[[hemi]][[sulcus]]
      max = rbind(max, data.frame(points=points, hemi=hemi, sulcus=sulcus))
    }
  }
...
#increment them
  for(i in seq(1, abs(diff))) {
    val = max[i,]
    ref.m.a2[[val[[2]]]][[val[[3]]]] = ref.m.a2[[val[[2]]]][[val[[3]]]] - 1
    ref.m.c[[val[[2]]]][[val[[3]]]] = ref.m.c[[val[[2]]]][[val[[3]]]] - 1
    }
  
# check landmarks
  num.m.c = sumlm(ref.m.c)
  print(num.m.c - nrow(GPA.c2$consensus)) #should be 0

```

# Second GPA
```{r}
resampled.m.a = resampleCurves(mSulc.a, surface, 'macaque', sulcalLengths=ref.m.a2)
num.m.a = sumlm(ref.m.a2)
data.m.a = transformToArray(resampled.m.a,num.m.a)

resampled.h.a = resampleCurves(hSulc.a, surface, 'human', sulcalLengths=ref.h.a2)
num.h.a = sumlm(ref.h.a2)
data.h.a = transformToArray(resampled.h.a, num.h.a)

GPA.h.a = procrustes(num.h.a, data.h.a, sulcalLengths=ref.h.a2)
GPA.m.a = procrustes(num.m.a, data.m.a, sulcalLengths=ref.m.a2)

```


#Separate Unique and Combined Sulci
```{r}
#Get list of Unique lm numbers MACAQUE
m.ss = c()
m.c = c()
unique = getSulci(surface, 'macaque', oneSpec=T)
i = 1
...
#find indices
for (hemi in sort(unlist(keys(ref.m.a2)))) {
      for (sulcus in sort(unlist(keys(ref.m.a2[[hemi]])))) {
        if (sulcus %in% unique){
          m.ss = c(m.ss, seq(i, i + ref.m.a2[[hemi]][[sulcus]] - 1)) #add rows to unique
        } else {
          m.c = c(m.c, seq(i, i + ref.m.a2[[hemi]][[sulcus]] -1 )) #add rows to combined
        }
        
        i = i + ref.m.a2[[hemi]][[sulcus]]
      }
    }

# cut to unique df
GPA.m.u = GPA.m.a$consensus[m.ss,]
GPA.m.c = GPA.m.a$consensus[m.c,]

#Get list of Unique lm numbers HUMAN
h.ss = c()
h.c = c()
unique = getSulci(surface, 'human', oneSpec=T)
i = 1
...
#find indices
for (hemi in sort(unlist(keys(ref.h.a2)))) {
      for (sulcus in sort(unlist(keys(ref.h.a2[[hemi]])))) {
        if (sulcus %in% unique){
          h.ss = c(h.ss, seq(i, i + ref.h.a2[[hemi]][[sulcus]] - 1)) #add rows to unique
        } else {
          h.c = c(h.c, seq(i, i + ref.h.a2[[hemi]][[sulcus]] - 1)) #add rows to combined
        }
        
        i = i + ref.h.a2[[hemi]][[sulcus]]
      }
    }

# cut to unique df
GPA.h.u = GPA.h.a$consensus[h.ss,]
GPA.h.c = GPA.h.a$consensus[h.c,]
...
transform.m = computeTransform(GPA.c2$consensus, GPA.m.c, type = "affine")
transform.h = computeTransform(GPA.c2$consensus, GPA.h.c, type = "affine")


coords.m = applyTransform(GPA.m.u, transform.m)
coords.h = applyTransform(GPA.h.u, transform.h) 
